KIRAMStoRunData <- function(runlogFile, resultFile = NULL){
    ...
    DFfromRunlogFile <- function(inFile){
        ...
        colTypes <- "cccciicciddddiidddddddddddd"
        df <- read_fwf(inFile, colWidths, skip=6, col_types=colTypes)
        
        colNames <- c("E","item","dateTime","grp","pos","meas","smType",
                      "label","numCycles","le12C","le13C","he12C","he13C",
                      "cntTotS","cntTotGT","le13.12","he13.12","he14.12",
                      "he14.13","trans12C","gvm","stripPR","S1ovnTR","S2ovnTR",
                      "FacTR","LEmagFld","HEmagFld")
        names(df) <- colNames
        ...
        batch <- str_extract(readLines(inFile, n=3)[3], '\\d\\d\\d\\d-[^\\s]*')
